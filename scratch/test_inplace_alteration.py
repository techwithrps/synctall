...
def main():
    ...
    print("1. Updating allocations to sum to 20,000.00...")
    ...
    for alloc in res.data:
        if alloc['fee_head'] == "TUTION FEE":
            client.client.table("fee_allocations").update({"amount": 15000.00}).eq("id", alloc['id']).execute()
    ...
    print("\n2. Updating transaction amount to 20,000.00...")
